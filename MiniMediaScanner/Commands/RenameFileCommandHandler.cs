...
public class RenameFileCommandHandler
{
    ...
    public RenameFileCommandHandler(string connectionString)
    {
        ...
        string directorySeperator = "_")
    {
        ...
        await ParallelHelper.ForEachAsync(metadatas, 4, async metadata =>
        {
            try
            {
                bool success = await ProcessFileAsync(metadata, overwrite,
                    subDirectoryDepth, fileFormat, directoryFormat, directorySeperator);
